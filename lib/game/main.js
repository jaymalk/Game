...
MyGame = ig.Game.extend({
	...
	draw: function() {
		...
		this.statText.draw("Lives", realx+5,realy+5);
		for(var i=0; i < this.lives; i++)
			this.lifeSprite.draw(((this.lifeSprite.width + 2) * i)+5+realx, 15+realy);
		// Showing level transit status
		if (this.showStats) {
			this.statMatte.draw(realx,realy);
			var x = ig.system.width/2;
			var y = 4*ig.system.height/6 - 40;
	...
	// Game Over Screen
	gameOver: function(){
		ig.finalStats = ig.game.stats; ig.system.setGame(GameOverScreen);
	}
});
